...
const useSettingsStore = create(
  persist(
    (set, get) => ({
      ...
      toggleFileSize: () =>
        set((state) => ({ showFileSize: !state.showFileSize })),
      toggleModifiedDate: () =>
        set((state) => ({ showModifiedDate: !state.showModifiedDate })),
      toggleCreatedDate: () =>
        set((state) => ({ showCreatedDate: !state.showCreatedDate })),
      toggleFileType: () =>
        set((state) => ({ showFileType: !state.showFileType })),
      toggleFullPath: () =>
        set((state) => ({ showFullPath: !state.showFullPath })),

      // --- NEW: Toggle Actions for Photo Mode ---
      toggleCamera: () => set((state) => ({ showCamera: !state.showCamera })),
      toggleLens: () => set((state) => ({ showLens: !state.showLens })),
      toggleISO: () => set((state) => ({ showISO: !state.showISO })),
      toggleAperture: () =>
        set((state) => ({ showAperture: !state.showAperture })),
      toggleShutter: () =>
        set((state) => ({ showShutter: !state.showShutter })),
      toggleDimensions: () =>
        set((state) => ({ showDimensions: !state.showDimensions })),
      toggleLocation: () =>
        set((state) => ({ showLocation: !state.showLocation })),
      // ------------------------------------------
      ...
      setRequestTimeoutMs: (ms) =>
        set({ requestTimeoutMs: Math.max(0, Number(ms) || 0) }),
      toggleDisableRequestTimeout: () =>
        set((state) => ({
          disableRequestTimeout: !state.disableRequestTimeout,
        })),
      getRequestTimeout: () => {
        const s = get();
      ...
      getNextOutputNameForPreset: (presetName, baseName) => {
        ...
        const nextIndex =
          current > 0 ? current + 1 : startFrom > 0 ? startFrom + 1 : 1;
